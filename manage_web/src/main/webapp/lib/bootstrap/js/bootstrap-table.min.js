...
! function(a) {
	...
	m.DEFAULTS = {
		...
		})), this.options.search && ( /*g=[],g.push('<div class="pull-'+this.options.searchAlign+' search">',c('<input class="form-control'+(void 0===this.options.iconSize?"":" input-"+this.options.iconSize)+'" type="text" placeholder="%s">',this.options.formatSearch()),"</div>"),*/ this.$toolbar.append(g.join("")), d = this.$container.find("#lse"), d.off("change").on("change", function(a) {
			//自定义的搜索方式，搜索方式为下拉框，ID为固定写死
			clearTimeout(i), i = setTimeout(function() {
				f.onSearch(a,"UNIT_CODE")//把要搜索的列名传过去
			}, f.options.searchTimeOut)
		}), "" !== this.options.searchText && (d.val(this.options.searchText), f.onSearch({
			currentTarget: d
		})))||( /*g=[],g.push('<div class="pull-'+this.options.searchAlign+' search">',c('<input class="form-control'+(void 0===this.options.iconSize?"":" input-"+this.options.iconSize)+'" type="text" placeholder="%s">',this.options.formatSearch()),"</div>"),*/ this.$toolbar.append(g.join("")), d = this.$container.find("#lse1"), d.off("change").on("change", function(a) {
			clearTimeout(i), i = setTimeout(function() {
				f.onSearch(a,"IS_TYPING")
			}, f.options.searchTimeOut)
		}), "" !== this.options.searchText && (d.val(this.options.searchText), f.onSearch({
			currentTarget: d
		})))
	}, m.prototype.onSearch = function(b,cc) {
		var c = a.trim(a(b.currentTarget).val());
		
		this.options.trimOnSearch && a(b.currentTarget).val() !== c && a(b.currentTarget).val(c), c !== this.searchText && (this.searchText = c, this.options.pageNumber = 1, this.initSearch(cc), this.updatePagination(), this.trigger("search", c))
	}, m.prototype.initSearch = function(cc) {
		if(cc=="UNIT_CODE"){//判断是哪个下拉框搜索，并获取列名
			var b = this;
			var dd=$("#lse1").val();//获取另一个下拉框目前选中的值
			...
			//第二个下拉框根据第一个搜索出的结果在进行搜索
			if("server" !== this.options.sidePagination) {
				var c = dd,
					d = a.isEmptyObject(this.filterColumns) ? null : this.filterColumns;
				this.data = d ? a.grep(this.data, function(a) {//这里是this.data前面搜索出的结果,不是this,options.data全部的值
					for(var b in d)
						if(a[b] !== d[b]) return !1;
					return !0
				}) : this.data, this.data = c ? a.grep(this.data, function(d, f) {
						g = "IS_TYPING";
						var i = d[g],
							j = b.columns[e(b.columns, g)],
							k = a.inArray(g, b.header.fields);
		...
		if("server" !== this.options.sidePagination) {
			var c = dd,
				d = a.isEmptyObject(this.filterColumns) ? null : this.filterColumns;
			this.data = d ? a.grep(this.data, function(a) {
				for(var b in d)
					if(a[b] !== d[b]) return !1;
				return !0
			}) : this.data, this.data = c ? a.grep(this.data, function(d, f) {
		...
		for(var k = this.pageFrom - 1; k < this.pageTo; k++) {
			var l, m = i[k],
				...
				q = {},
				r = [];
			if(n = h(this.options, this.options.rowStyle, [m, k], n), n && n.css)
				for(l in n.css) o.push(l + ": " + n.css[l]);
			if(q = h(this.options, this.options.rowAttributes, [m, k], q))
				for(l in q) r.push(c('%s="%s"', l, j(q[l])));
		...
		b = this.$tableBody.get(0), d = b.scrollWidth > b.clientWidth && b.scrollHeight > b.clientHeight + this.$header.outerHeight() ? g() : 0, this.$el.css("margin-top", -this.$header.outerHeight()), this.$header_ = this.$header.clone(!0, !0), this.$selectAll_ = this.$header_.find('[name="btSelectAll"]'), this.$tableHeader.css({
			"margin-right": d
	...
	a.fn.bootstrapTable = function(b) {
		...
		return this.each(function() {
			var e = a(this),
				...
				g = a.extend({}, m.DEFAULTS, e.data(), "object" == typeof b && b);
			if("string" == typeof b) {
